...
#
bb2=$(cat /var/log/glusterfs/bricks/mnt-gbrick2g-subdir.log |grep args|tail -n 1|awk -F: '{print $8}'|rev|sed 's/^.//'|rev|sed 's/^.//'|cut -c 22-)
bb1=$(cat /var/log/glusterfs/bricks/mnt-gbrick-subdir.log |grep args|tail -n 1|awk -F: '{print $8}'|rev|sed 's/^.//'|rev|sed 's/^.//'|cut -c 22-)
tti=$(cat /var/log/glusterfs/tier/myvol1/tierd.log |grep args|tail -n 1|awk -F: '{print $8}'|rev|sed 's/^.//'|rev|sed 's/^.//'|cut -c 21-)
gfsd='/usr/sbin/glusterfsd'
gfs='/usr/sbin/glusterfs'
